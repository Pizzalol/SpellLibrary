...
================================================================================================================= ]]
function item_mekansm_on_spell_start(keys)	
	...
	for i, nearby_ally in ipairs(nearby_allied_units) do  --Restore health and play a particle effect for every found ally.
		if not nearby_ally:HasModifier("item_mekansm_modifier_heal_debuff") then
		...
		keys.ability:ApplyDataDrivenModifier(keys.caster, nearby_ally, "item_mekansm_modifier_heal_armor", nil)
		keys.ability:ApplyDataDrivenModifier(keys.caster, nearby_ally, "item_mekansm_modifier_heal_debuff", nil)
	end
end