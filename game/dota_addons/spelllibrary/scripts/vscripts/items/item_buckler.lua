...
================================================================================================================= ]]
function item_buckler_on_spell_start(keys)	
	...
	for i, nearby_ally in ipairs(nearby_allied_units) do
		if nearby_ally:IsHero() then
			keys.ability:ApplyDataDrivenModifier(keys.caster, nearby_ally, "item_buckler_modifier_active", {duration = keys.ActiveDurationHero})
		else
			keys.ability:ApplyDataDrivenModifier(keys.caster, nearby_ally, "item_buckler_modifier_active", {duration = keys.ActiveDurationUnit})
		end
	end
end